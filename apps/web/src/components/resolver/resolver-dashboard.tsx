...
export function ResolverDashboard({ initialMarkets }: ResolverDashboardProps) {
  ...
  const handleResolve = useCallback(
    async (market: MarketViewModel, outcome: OutcomeSide) => {
      ...
      try {
        if (!walletClient?.account || !publicClient || market.onChainMarketId === undefined) {
          throw new Error("Connect the resolver wallet to Arc Mainnet first.");
        }
        const hash = await walletClient.writeContract({
          ...
          args: [market.onChainMarketId, outcome === "yes" ? 1 : 2],
          chain: arcMainnet,
        });
    ...
    [publicClient, refresh, walletClient]
  );
