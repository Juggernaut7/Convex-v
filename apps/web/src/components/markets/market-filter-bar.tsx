  ...
  return (
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex w-full flex-col gap-2 sm:flex-row">
        <div className="flex flex-1 items-center gap-2 rounded-xl border border-zinc-800 bg-secondary/50 px-3">
          <Search className="h-4 w-4 text-muted-foreground" />
          <Input
            value={search}
            onChange={(event: ChangeEvent<HTMLInputElement>) => handleSearchChange(event.target.value)}
            className="h-10 border-0 bg-transparent px-0 text-sm text-white placeholder:text-muted-foreground focus:ring-0 focus-visible:ring-0"
        ...
        {showFiltersButton && (
          <Button
            variant="outline"
            className="flex items-center gap-2 rounded-xl border-zinc-800 bg-secondary/50 text-sm font-semibold text-zinc-300 hover:bg-secondary sm:w-auto"
          >
            ...
            <button
              ...
              className={cn(
                ...
                activeCategory === categoryOption
                  ...
                  : "bg-secondary/70 text-zinc-400 hover:text-white"
              )}
