...
contract ConvexMarketManager is AccessControl, Pausable, ReentrancyGuard {
    ...
    function stake(uint32 marketId, Outcome side, uint128 amount) external payable whenNotPaused nonReentrant {
    ...
    function resolveMarket(uint32 marketId, Outcome outcome) external whenNotPaused {
        ...
        if (protocolFee > 0) {
                _sendNative(treasury, protocolFee);
        }
        if (creatorFee > 0) {
            _sendNative(market.creator, creatorFee);
        }
